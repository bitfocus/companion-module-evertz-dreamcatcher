...
class DC extends base_1.InstanceBase {
    constructor() {
        super(...arguments);
        this.requestId = 1;
        this.pending = new Map();
        this.pendingHotkeys = new Map();
        this.pendingTags = new Map();
        this.pendingSessionClipRefresh = new Map();
        this.pendingSessionPbs = new Map();
        this.outputs = [];
        this.inputs = [];
        this.bins = [];
        this.exportProfiles = [];
    ...
    connect() {
        ...
        this.updateStatus(base_1.InstanceStatus.Connecting);
        this.tcp = new base_1.TCPHelper(this.configData.host, this.configData.port, { reconnect: true });
        this.tcp.on('connect', () => {
    ...
    handleResponse(msg) {
        ...
        if (method === 'get_sessions' && Array.isArray(msg.result)) {
            this.sessions = msg.result
                ...
                clipIds: Array.isArray(s.session_clips) ? s.session_clips.map((clipId) => String(clipId)) : [],
            }))
    ...
    handleTagClipLookup(requestId, result) {
    ...
    extractArray(value) {
        ...
        if (Array.isArray(value?.playlist))
            return value.playlist;
        if (Array.isArray(value?.local_playlists))
            return value.local_playlists;
        if (Array.isArray(value?.localPlaylists))
            return value.localPlaylists;
    ...
    defaultSessionClipId() {
        return this.sessionClips[0]?.id || '';
    }
    refreshSessionClips(sessionId) {
        if (!sessionId) {
        ...
        this.httpJsonRpc('find_clips', { session_id: sessionId, limit: 500 }, (clipsResult) => {
            ...
            this.sessionClips = rawClips
                ...
                if (clip.placeholder === true)
                    return false;
                if (clip.primary === false)
                    return false;
                if (clip.clip_id && clip.clip_id === clip.record_train_id)
                    return false;
                if (clip.name && /^Input\d+$/i.test(String(clip.name)))
                    return false;
                return true;
            })
                .map((clip, index) => {
    ...
    getSelectedMask(outputId) {
        if (outputId === '__ALL__')
            return this.outputs.reduce((mask, output) => mask + output.mask, 0);
        if (outputId === '__FIRST_TWO__')
            return this.outputs.slice(0, 2).reduce((mask, output) => mask + output.mask, 0);
        const output = this.outputs.find((o) => o.id === outputId);
        return output ? output.mask : null;
    }
    getSelectedOutputIds(outputId) {
        if (outputId === '__ALL__')
            return this.outputs.map((o) => o.id);
        if (outputId === '__FIRST_TWO__')
            return this.outputs.slice(0, 2).map((o) => o.id);
        return outputId ? [outputId] : [];
    }
    handleSessionPbsClipLookup(requestId, result) {
        const request = this.pendingSessionPbs.get(requestId);
        if (!request)
            return;
        const clips = this.extractArray(result);
        const clip = clips.find((c) => String(c.readable_id || '') === request.readableId) || clips[0];
        if (!clip?.clip_id) {
    ...
    requestSessionPbsClipLookup(sessionId, readableId, outputIds, mask, speed, playAfterCue) {
    ...
    requestClipHotkeyLookup(readableId, outputIds, mask, speed, playAfterCue) {
        const id = this.requestId++;
        const msg = {
            jsonrpc: '2.0',
            method: 'find_clips',
    ...
    requestTagClipLookup(readableId, tag, applyToGroup) {
        ...
        return {
            ...
            route_input_to_output: {
                ...
                callback: async (event) => {
                    ...
                    if (!inputId)
                        return this.log('warn', 'No input selected/discovered');
                    if (outputIds.length === 0)
                        return this.log('warn', 'No output selected/discovered');
            ...
            cue_session_clip_by_pbs: {
                ...
                callback: async (event) => {
                    ...
                    if (outputIds.length === 0)
                        return this.log('warn', 'No output selected/discovered');
                    if (!sessionId)
                        return this.log('warn', 'No session selected. Run Refresh Sessions first.');
                    const readableId = this.buildReadableId(String(event.options.page || 1), String(event.options.bank || 1), String(event.options.slot || 1), String(event.options.angle || 'A'));
            ...
            create_clip: {
                ...
                callback: async (event) => {
                    ...
                    this.login();
                    this.selectOutputs(mask);
                    const params = { name: String(event.options.name || '%C_%L_%i_%o') };
                    const readableId = String(event.options.readable_id || '').trim();
                    if (readableId)
                        params.readable_id = readableId;
            ...
            recall_last_clip: {
                ...
                callback: async (event) => {
                    if (!this.lastClipId)
                        return this.log('warn', 'No last clip stored yet');
                    const outputIds = this.getSelectedOutputIds(String(event.options.output || ''));
                    if (outputIds.length === 0)
                        return this.log('warn', 'No output selected/discovered');
            ...
            restart_components: {
                ...
                callback: async (event) => {
                    ...
                    const params = {};
                    if (event.options.inputs)
                        params.inputs = true;
                    if (event.options.outputs)
                        params.outputs = true;
                    if (event.options.amps)
                        params.amps = true;
                    if (event.options.vue)
                        params.vue = true;
    ...
    getPresets() {
        ...
        return {
            restart_all_components: {
                ...
                style: { text: 'RESTART\nALL', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(160, 0, 0) },
            ...
            reboot_server: {
                ...
                category: 'System Restart',
                name: 'Reboot Server',
                style: { text: 'REBOOT\nSERVER', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(180, 0, 0) },
            ...
            restart_inputs: {
                ...
                category: 'System Restart',
                name: 'Restart Inputs',
                style: { text: 'RESTART\nINPUTS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(130, 70, 0) },
            ...
            restart_outputs: {
                ...
                style: { text: 'RESTART\nOUTPUTS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(130, 70, 0) },
            ...
            restart_amps: {
                ...
                style: { text: 'RESTART\nAMP', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(130, 70, 0) },
            ...
            restart_vue: {
                ...
                category: 'System Restart',
                name: 'Restart VUE',
                style: { text: 'RESTART\nVUE', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(130, 70, 0) },
            ...
            refresh_outputs: {
                ...
                category: 'System',
                name: 'Refresh Outputs',
                style: { text: 'REFRESH\nOUTPUTS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(50, 50, 50) },
            ...
            refresh_inputs: {
                ...
                category: 'System',
                name: 'Refresh Inputs',
                style: { text: 'REFRESH\nINPUTS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(50, 50, 50) },
            ...
            refresh_bins: {
                ...
                category: 'System',
                name: 'Refresh Bins',
                style: { text: 'REFRESH\nBINS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(50, 50, 50) },
            ...
            refresh_export_profiles: {
                ...
                category: 'System',
                name: 'Refresh Export Profiles',
                style: { text: 'REFRESH\nEXPORTS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(50, 50, 50) },
            ...
            start_capture: {
                ...
                name: 'Start Capture',
                style: { text: 'START\nCAPTURE', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(0, 120, 0) },
            ...
            stop_capture: {
                ...
                category: 'Capture Control',
                name: 'Stop Capture',
                style: { text: 'STOP\nCAPTURE', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(160, 0, 0) },
            ...
            refresh_connection_preset: {
                ...
                category: 'System',
                name: 'Refresh Connection',
                style: { text: 'REFRESH\nCONNECTION', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(30, 70, 130) },
            ...
            play_out1: {
                ...
                name: 'Play Output 1',
                style: { text: 'PLAY\nOUT 1', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(0, 120, 0) },
            ...
            pause_out1: {
                ...
                category: 'Output Control',
                name: 'Pause Output 1',
                style: { text: 'PAUSE\nOUT 1', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(160, 110, 0) },
            ...
            scrub_back_out1: {
                ...
                style: { text: 'SCRUB\n-30', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(80, 80, 80) },
            ...
            live_out1: {
                ...
                style: { text: 'LIVE\nOUT 1', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(0, 75, 160) },
            ...
            goto_in: {
                ...
                category: 'Output Control',
                name: 'Goto In',
                style: { text: 'GOTO\nIN', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(70, 70, 140) },
            ...
            goto_out: {
                ...
                category: 'Output Control',
                name: 'Goto Out',
                style: { text: 'GOTO\nOUT', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(140, 70, 70) },
            ...
            mark_in: {
                ...
                name: 'Mark In',
                style: { text: 'MARK\nIN', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(0, 90, 160) },
            ...
            mark_out: {
                ...
                category: 'Clip Workflow',
                name: 'Mark Out',
                style: { text: 'MARK\nOUT', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(0, 90, 160) },
            ...
            create_clip: {
                ...
                category: 'Clip Workflow',
                name: 'Create Clip',
                style: { text: 'CREATE\nCLIP', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(140, 0, 140) },
            ...
            recall_last_clip: {
                ...
                category: 'Clip Workflow',
                name: 'Recall Last Clip',
                style: { text: 'RECALL\nLAST', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(90, 0, 140) },
            ...
            refresh_sessions: {
                ...
                category: 'System',
                name: 'Refresh Sessions',
                style: { text: 'REFRESH\nSESSIONS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(50, 50, 50) },
            ...
            refresh_session_clips: {
                ...
                category: 'System',
                name: 'Refresh Session Clips',
                style: { text: 'REFRESH\nSESSION CLIPS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(50, 50, 50) },
            ...
            cue_session_clip_by_pbs_preset: {
                ...
                name: 'Cue Session Clip by PBS',
                style: { text: 'CUE SESSION\nPBS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(45, 45, 120) },
            ...
            play_session_clip_by_pbs_preset: {
                ...
                category: 'Clip Hotkeys',
                name: 'Play Session Clip by PBS',
                style: { text: 'PLAY SESSION\nPBS', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(0, 100, 70) },
            ...
            cue_clip_hotkey_preset: {
                ...
                style: { text: 'CUE\nCLIP NAME', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(45, 45, 120) },
            ...
            play_clip_hotkey_preset: {
                ...
                style: { text: 'PLAY\nCLIP NAME', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(0, 100, 70) },
            ...
            route_input_out1: {
                ...
                style: { text: 'INPUT\nTO OUT 1', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(0, 90, 120) },
            ...
            mosaic_custom_2box: {
                ...
                style: { text: 'MOSAIC\n2 BOX', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(60, 60, 130) },
            ...
            mosaic_custom_3box: {
                ...
                style: { text: 'MOSAIC\n3 BOX', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(60, 60, 130) },
                steps: [{ down: [{ actionId: 'set_mosaic_custom_layout', options: {
                                    output: out1,
                                    box_count: '3',
                                    border_thickness: 0,
            ...
            mosaic_custom_4box: {
                ...
                style: { text: 'MOSAIC\n4 BOX', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(60, 60, 130) },
                steps: [{ down: [{ actionId: 'set_mosaic_custom_layout', options: {
                                    output: out1,
                                    box_count: '4',
                                    border_thickness: 0,
            ...
            mosaic_clear: {
                ...
                category: 'Mosaic',
                name: 'Clear Mosaic',
                style: { text: 'CLEAR\nMOSAIC', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(80, 80, 80) },
            ...
            black_out1: {
                ...
                category: 'Output Control',
                name: 'Black Output 1',
                style: { text: 'BLACK\nOUT 1', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(20, 20, 20) },
            ...
            play_first_two: {
                ...
                style: { text: 'PLAY\nOUT 1+2', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(0, 120, 0) },
            ...
            black_first_two: {
                ...
                style: { text: 'BLACK\nOUT 1+2', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(20, 20, 20) },
            ...
            black_all: {
                ...
                style: { text: 'BLACK\nALL', size: '14', color: (0, base_1.combineRgb)(255, 255, 255), bgcolor: (0, base_1.combineRgb)(20, 20, 20) },
    ...
    httpJsonRpc(method, params, callback) {
